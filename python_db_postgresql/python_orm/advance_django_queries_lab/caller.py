...
def product_quantity_ordered() -> str:
    total_ordered_products_quantity = Product.objects.annotate(
        total_quantity_products=Sum('orderproduct__quantity')).exclude(total_quantity_products=None).order_by(
        '-total_quantity_products')

    result = []

    for product in total_ordered_products_quantity:
        result.append(f'Quantity ordered of {product.name}: {product.total_quantity_products}')
